...
vasServiceSchema.statics.getActiveServices = async function () {
  try {
    return await this.find({ status: "active" }).sort({
      popularity: -1,
      createdAt: -1,
    });
  } catch (error) {
...
vasServiceSchema.statics.getServicesByCategory = async function (category) {
  try {
    ...
    console.error(
      `Error fetching VAS services for category ${category}:`,
      error,
    );
    throw error;
  }
...
vasServiceSchema.statics.searchServices = async function (
  searchTerm,
  filters = {},
) {
  try {
...
vasServiceSchema.pre("remove", async function (next) {
  try {
    ...
    console.warn(
      `VAS Service ${this.name} (${this.id}) is being deleted. Consider handling existing subscriptions.`,
    );
    next();
  } catch (error) {
    next(error);
