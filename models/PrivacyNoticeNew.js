...
const privacyNoticeSchema = new mongoose.Schema(
  {
    ...
    content: {
      ...
      // Allow both string and nested object structure
      required: true,
    },
    contentType: {
      type: String,
...
privacyNoticeSchema.pre("save", function (next) {
  ...
  if (!this.nextReviewDate && this.effectiveDate) {
    this.nextReviewDate = new Date(
      this.effectiveDate.getTime() + 365 * 24 * 60 * 60 * 1000,
    );
  }
...
// Instance methods
privacyNoticeSchema.methods.acknowledge = function (
  userId,
  userEmail,
  metadata = {},
) {
...
const PrivacyNotice =
  mongoose.models["PrivacyNotice"] ||
  mongoose.model("PrivacyNotice", privacyNoticeSchema);
