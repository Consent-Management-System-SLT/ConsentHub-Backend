...
const dsarRequestSchema = new mongoose.Schema(
  {
    ...
    requestId: {
      ...
      unique: true,
      default: () =>
        `DSAR-${Date.now()}-${Math.random().toString(36).substr(2, 6).toUpperCase()}`,
    },
...
dsarRequestSchema.statics.getRequestsByDateRange = function (
  startDate,
  endDate,
) {
  return this.find({
...
dsarRequestSchema.methods.addCommunication = function (
  type,
  direction,
  content,
  author,
