  ...
  try {
    ...
    channelStats.forEach((stat) => {
      channels[stat._id] = {
        ...
        openRate:
          stat.delivered > 0
            ? ((stat.opened / stat.delivered) * 100).toFixed(1)
            : 0,
        clicked: stat.clicked,
        clickRate:
          stat.opened > 0 ? ((stat.clicked / stat.opened) * 100).toFixed(1) : 0,
        failed: stat.failed,
      };
    });
    return {
      overview: {
        ...
        deliveryRate:
          overall.totalSent > 0
            ? ((overall.totalDelivered / overall.totalSent) * 100).toFixed(1)
            : 0,
        openRate:
          overall.totalDelivered > 0
            ? ((overall.totalOpened / overall.totalDelivered) * 100).toFixed(1)
            : 0,
        clickRate:
          overall.totalOpened > 0
            ? ((overall.totalClicked / overall.totalOpened) * 100).toFixed(1)
            : 0,
      },
      channels,
...
NotificationLogSchema.post("save", function (doc, next) {
  console.log(
    ` NotificationLog saved: ${doc.channel} to ${doc.customerName} (${doc.status})`,
  );
  next();
});
